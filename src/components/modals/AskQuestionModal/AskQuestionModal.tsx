...
import { CREATE_LIVE_COMMENT } from "../../../gql/mutation/CreateLiveComment";

const emailFormat = /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;

const AskQuestionModal = ({ closeModal, btnCancelClick, streamId }: any) => {
  ...
  async function handleSubmit() {
    if (values.text.length > 0) {
      try {
        ...
        if (response.data.createLiveStreamsComment) {
