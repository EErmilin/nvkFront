...
const isMobile = /iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
  navigator.userAgent
);
...
const VideoPlayer = forwardRef(
  (
    ...
    const toggleSettings = () => setIsOpenSettings(!isOpenSettings);
    ...
    let clsWrapper = [
      isMain ? "video-pleer-wraper-main" : "video-pleer-wraper",
    ];
    if (isFull) {
      clsWrapper = [];
    ...
    if (!steam) {
      return (
        <div>
          <div className={cls.join(" ")} style={{ aspectRatio: 16 / 9 }}>
            {children}
          </div>
        </div>
      );
    ...
    const renderQuality = () => {
      const arr = steam.media?.hls
        .sort(function (a, b) {
          return parseInt(b.name ?? "0", 10) - parseInt(a.name ?? "0", 10);
        })
        .map((item) => {
          return {
            name: item.name,
    ...
    return (
      <>
        <div className={clsWrapper.join(" ")} ref={videoPleerWraper}>
          {isOpenSettings ? (
            <div className="settings">{renderQuality()}</div>
          ) : (
            <ReactPlayer
              ref={playerRef}
              url={video}
        ...
        {isShowBtn && (
          <VideoInfo
            streamTitle={steam?.name}
            programTitle={programTitle}
            askButtonClick={onAsk}
          />
        )}
      </>
    );
